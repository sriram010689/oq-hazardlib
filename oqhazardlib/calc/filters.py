...
"""
Module :mod:`~oqhazardlib.calc.filters` contain filter functions for
calculators.
...
1. Source-site filters. Those functions take a generator of two-item tuples,
   ...
   a subclass of :class:`~oqhazardlib.source.base.SeismicSource`) and a
   site collection (instance of
   :class:`~oqhazardlib.site.SiteCollection`).
2. Rupture-site filters. Those also take a generator of pairs, but in this
   ...
   :class:`~oqhazardlib.source.rupture.Rupture`). The second element in
...
<oqhazardlib.site.SiteCollection.filter>` site collection.
...
Module :mod:`oqhazardlib.calc.filters` exports one distance-based
...
def source_site_distance_filter(integration_distance):
    ...
    :param integration_distance:
        ...
        :meth:`oqhazardlib.source.base.SeismicSource.filter_sites_by_distance_to_source`
...
def rupture_site_distance_filter(integration_distance):
    ...
    :param integration_distance:
        Threshold distance in km, this value gets passed straight to
        :meth:`oqhazardlib.source.base.SeismicSource.filter_sites_by_distance_to_rupture`
        which is what is actually used for filtering.
    """
