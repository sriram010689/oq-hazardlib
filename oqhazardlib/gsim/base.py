...
"""
Module :mod:`oqhazardlib.gsim.base` defines base classes for
...
from oqhazardlib import imt as imt_module
...
class GroundShakingIntensityModel(object):
    ...
    #: <oqhazardlib.const.IMC>` this GSIM can calculate mean
    #: and standard
    #: deviation for.
    DEFINED_FOR_INTENSITY_MEASURE_COMPONENT = abc.abstractproperty()

    #: Set of
    #: :class:`standard deviation types <oqhazardlib.const.StdDev>`
    ...
    #:     in decimal degrees. See :mod:`~oqhazardlib.geo.nodalplane`
    #:     for more detailed description of dip and rake.
    #: ``ztor``
    #:     Depth of rupture's top edge in km. See
    #:     :meth:`~oqhazardlib.geo.surface.base.BaseSurface.get_top_edge_depth`.
    #:
    ...
    #:     :meth:`~oqhazardlib.geo.surface.base.BaseSurface.get_min_distance`.
    #: ``rjb``
    #:     Distance to rupture's surface projection. See
    #:     :meth:`~oqhazardlib.geo.surface.base.BaseSurface.get_joyner_boore_distance`.
    #: ``rx``
    #:     Perpendicular distance to rupture top edge projection.
    #:     See :meth:`~oqhazardlib.geo.surface.base.BaseSurface.get_rx_distance`.
    #:
    ...
    def get_mean_and_stddevs(self, sites, rup, dists, imt, stddev_types):
        ...
        :param imt:
            ...
            See :mod:`oqhazardlib.imt`.
        :param stddev_types:
            List of standard deviation types, constants from
            :class:`oqhazardlib.const.StdDev`.
    ...
    def get_poes(self, sctx, rctx, dctx, imt, imls, truncation_level):
        ...
        :param imt:
            ...
            of classes from :mod:`oqhazardlib.imt`).
        :param imls:
    ...
    def make_contexts(self, site_collection, rupture):
        ...
        :param site_collection:
            Instance of :class:`oqhazardlib.site.SiteCollection`.

        :param rupture:
            Instance of
            :class:`~oqhazardlib.source.rupture.Rupture` (or
            its subclass
            :class:`~oqhazardlib.source.rupture.ProbabilisticRupture`).
...
class CoeffsTable(object):
    ...
    If there are :class:`~oqhazardlib.imt.SA` IMTs in the table, they
    ...
    by instances of :class:`oqhazardlib.imt.SA` with period
    ...
    def __getitem__(self, imt):
        ...
        of type :class:`~oqhazardlib.imt.SA` and interpolation
        is possible.
